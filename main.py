...
def opening_statements(): # Clear output and display program information
    clear()
    print("-- Northrup E-Mount Serial Number Generator --\n")
    print("This program generates serial number GCODE files for the Mazak Variaxis i-700.")
...
def get_first_serial()->int:
    try:
        ...
        if LOWEST_NUMBER <= first_serial <= HIGHEST_NUMBER: # Make sure first_serial is between 0000 and 9999
            return first_serial # Return it if everything looks good
        else:
            opening_statements()
            print(INPUT_START_RANGE_ERROR_MESSAGE)
            return get_first_serial() # If not, rerun get_first_serial(), this time with the error message displayed
...
def get_last_serial(first_serial: int)->int:
    try:
        ...
        if first_serial <= last_serial <= HIGHEST_NUMBER: # Make sure the last serial number is greater-than or equal-to the first serial number
            return last_serial # Return it if everything looks good
        else:
            opening_statements()
            print(f"{FIRST_SERIAL_INPUT_MESSAGE}{first_serial}\n")
            print(INPUT_END_RANGE_ERROR_MESSAGE + str(first_serial) + f" and {HIGHEST_NUMBER}")
            return get_last_serial(first_serial)
